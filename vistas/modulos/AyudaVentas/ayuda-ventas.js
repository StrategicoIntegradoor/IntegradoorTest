    ...
    .then(data => {
        ...
        data.forEach(ayudaVenta => {
            ...
            if (ayudaVenta.link_clausulado) {
                partTemplate += `<td style="margin-top: 60px;"><button class="btn btn-alert" style="border-color: #88d600; width: 205px; color: #88d600; font-weight: 500;" onclick="validarPermisoClausulado('${ayudaVenta.link_clausulado}')">${ayudaVenta.link_clausulado.substring(0,27)}</button></td>`
            } else {
                partTemplate += '<td></td>'
            }
            if (ayudaVenta.path_sarlaft || ayudaVenta.path_sarlaft2) {
                let sarlaftButtons = '<td style="margin-top: 60px;">'
                ...
                let sarlaftButtons2 = '<td style="margin-top: 60px;">'
