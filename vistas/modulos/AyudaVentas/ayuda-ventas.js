    ...
    .then(data => {
        ...
        data.forEach(ayudaVenta => {
            ...
            if (ayudaVenta.link_clausulado) {
                partTemplate += `<td style="text-align: center;"><ul style="margin-top: 60px;"><button class="btn btn-alert" style="border-color: #88d600; width: 85%; color: #88d600; font-weight: 500;" onclick="validarPermisoClausulado('${ayudaVenta.link_clausulado}')">${ayudaVenta.link_clausulado.substring(0, 27)}</button></ul></td>`;
            } else {
                partTemplate += '<td></td>';
            }
